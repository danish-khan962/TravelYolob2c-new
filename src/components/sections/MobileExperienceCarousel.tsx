...
interface MobileExperienceCarouselProps {
...
const MobileExperienceCarousel: React.FC<MobileExperienceCarouselProps> = ({
  applySigTransforms,
  ...
  const handleNextSlide = () => {
    if (swiperRef.current) {
      const newIndex = Math.min(destinations.length - 1, activeIndex + 1);
      swiperRef.current.slideTo(newIndex);
    }
  };
...
export default MobileExperienceCarousel;
